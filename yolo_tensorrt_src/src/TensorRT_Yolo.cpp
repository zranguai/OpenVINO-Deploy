...
std::string TRT_YOLO_Imp::GetNowTime() {
...
TRT_YOLO_Imp::TRT_YOLO_Imp(int input_height, int input_width, int input_batch, float confidence_threshold, float nms_threshold) {
...
TRT_YOLO_Imp::~TRT_YOLO_Imp() {
	//delete _dl_input_instancev8_parm;
	//_dl_input_instancev8_parm = nullptr;
}

int TRT_YOLO_Imp::Get_BatchSize()
{
	return this->_input_batch;
}

std::vector<std::string> TRT_YOLO_Imp::Get_ClassName(std::string class_name_path)
{
...
int TRT_YOLO_Imp::Init_YOLO_Model(const char* model_path, const char* classes_path, const char* type)
{
	...
	try
	{
		...
		if (data.size() <= 0) {
			std::string msg = "Init_YOLO_Model: engine文件加载为空，请检查模型文件是否存在或是否正常导出!";
			myLogTrt.error(msg);
			return -1;
		}
		...
		if (strcmp(str1, type) == 0) {
			...
			std::string msg = "Init_YOLO_Model中，你传入的模型类型是:" + strtype + "!";
			std::cout << msg << std::endl;
			myLogTrt.info(msg);
		}
			...
			std::string msg = "Init_YOLO_Model中，你传入的模型类型是:" + strtype + "!" + "你应该输入YOLOV5Det，YOLOV8Det，YOLOV8Seg！";
			std::cout << msg << std::endl;
			myLogTrt.error(msg);
			return -1;
		}

		_fast_yolo_models = yolo::load_from_memory(data, model_type, _confidence_threshold, _nms_threshold);
		if (_fast_yolo_models == nullptr) {
			std::string msg = "Init_YOLO_Model: 初始化失败，模型为空!";
			std::cout << msg << std::endl;
			myLogTrt.error(msg);
			return -1;
		}
		...
		if (_class_names.size() <= 0) {
			myLogTrt.error("Init_YOLO_Model: 获取缺陷列表为空，请检查缺陷文件是否存在或是否正常配置!");
			return -1;
		}
		return 0;
	}
	...
	{
		std::string msg = "Init_Det_Model 捕获到异常: ";
		std::string strExcep = e.what();
		msg = msg + strExcep;
		std::cout << msg << std::endl;
		myLogTrt.error(msg);
...
std::string TRT_YOLO_Imp::Run_YOLO_Model(InputImageParm input_image_parm)
{
	try
	{
		if (_fast_yolo_models == nullptr) {
			std::string msg = "Run_YOLO_Model: 模型为空!";
			std::cout << msg << std::endl;
			myLogTrt.error(msg);
			return "";
		}
	...
	{
		std::string msg = "Run_YOLO_Model 捕获到异常: ";
		std::string strExcep = e.what();
		msg = msg + strExcep;
		std::cout << msg << std::endl;
		myLogTrt.error(msg);
...
void TRT_YOLO_Imp::Free_YOLO_Memoy() {
...
std::vector<std::vector<DLResultData>> TRT_YOLO_Imp::YoloDet(std::vector<sImage> input_mats, bool bSliceFlag)
{
...
std::vector<unsigned char> TRT_YOLO_Imp::Load_File(const std::string& file)
{
...
std::vector<sImage> TRT_YOLO_Imp::Get_Mats(InputImageParm input_parm)
{
...
int TRT_YOLO_Imp::Run_PreProcessingImage(InputImageParm input_image_parm, std::vector<sImage>& input_mats)
{
	input_mats.clear();
	try
	{
...
std::vector<std::vector<DLResultData>> TRT_YOLO_Imp::Infer_Predict(std::vector<sImage> input_imgs)
{
	//推理接口
		...
		for (int ib = 0; ib < (int)batched_result.size(); ++ib) {
			...
			if ((!objs.empty()) && batch_status[ib] == true) {//有缺陷数据时
				for (auto& obj : objs) {
					...
					if (obj.seg) {
						cv::Mat mask(obj.seg->height, obj.seg->width, CV_8U, obj.seg->data);  // 8位单通道图像
						cv::Mat binary_mask;
						cv::threshold(mask, binary_mask, 127, 255, cv::THRESH_BINARY);

						std::vector<std::vector<cv::Point>> contours;
						cv::findContours(binary_mask, contours, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);  // 找轮廓
						std::sort(contours.begin(), contours.end(), CompareContourAreas);  
						if (contours.size() != 0) {
							for (int c = 0; c < contours.size(); c++) {

								for (int n = 0; n < contours[c].size(); n++) {
									int xx = std::round(contours[c][n].x * 4 * ratio) + int(obj.left);  // 根据比例还原x、y坐标
									int yy = std::round(contours[c][n].y * 4 * ratio) + int(obj.top);
									contours[c][n] = cv::Point(xx, yy);
								}
							}
							////暂时屏蔽轮廓赋值(软件不需要)
							//single_output._contours = contours[0];
							single_output._area = cv::contourArea(contours[0]);  // 面积最大的轮廓
...
int TRT_YOLO_Imp::Get_TRT_Det_Result(std::vector<std::vector<DLResultData>> dl_data, int& result)
{
...
/*保留接口未使用，未测试*/
std::vector<DLResultData> TRT_YOLO_Imp::Merge_Det_Result(std::vector<std::vector<DLResultData>>slice_result, SliceMergeData slice_mats) {
...
int TRT_YOLO_Imp::VectorArrayCopy(std::vector<std::vector<DLResultData>> vSource, std::vector<std::vector<DLResultData>>& vDst)
{
