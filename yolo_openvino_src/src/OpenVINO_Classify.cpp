...
std::string OV_Classify_Imp::GetNowTime() {
...
OV_Classify_Imp::OV_Classify_Imp(int input_height, int input_width, int input_batch) {
...
OV_Classify_Imp::~OV_Classify_Imp() {
	//delete _dl_input_instancev8_parm;
	//_dl_input_instancev8_parm = nullptr;
}

int OV_Classify_Imp::Get_BatchSize()
{
	return this->_input_batch;
}

std::vector<std::string> OV_Classify_Imp::Get_ClassName(std::string class_name_path)
{
...
int OV_Classify_Imp::Init_Classify_Model(const char* model_path, const char* classes_path, const char* type)
{
	try
	{
		...
		_fast_classify_models = Classify::load_from_memory(model_path,_input_batch,_input_width,_input_height, _class_names.size());
...
std::string OV_Classify_Imp::Run_Classify_Model(InputImageParm input_image_parm)
{
	try
	{
		if (_fast_classify_models == nullptr) {
...
void OV_Classify_Imp::Free_Classify_Memoy() {
	if (_fast_classify_models != nullptr) {
...
std::vector<std::vector<DLResultData>> OV_Classify_Imp::ClassifyDet(std::vector<sImage> input_mats)
{
...
std::vector<sImage> OV_Classify_Imp::Get_Mats(InputImageParm input_parm)
{
...
int OV_Classify_Imp::Run_PreProcessingImage(InputImageParm input_image_parm, std::vector<sImage>& input_mats)
{
	input_mats.clear();
	try
	{
...
//分类不用分图
std::vector<std::vector<DLResultData>> OV_Classify_Imp::Infer_Predict(std::vector<sImage> input_imgs)
{
	//推理接口
...
int OV_Classify_Imp::Get_TRT_Classify_Result(std::vector<std::vector<DLResultData>> dl_data, int& result)
{
	...
	{
		std::string msg = "Get_OV_Classify_Result 捕获到异常: ";
		std::string strExcep = e.what();
		msg = msg + strExcep;
		std::cout << msg << std::endl;
		myLogTrt.error(msg);
...
int OV_Classify_Imp::VectorArrayCopy(std::vector<std::vector<DLResultData>> vSource, std::vector<std::vector<DLResultData>>& vDst)
{
